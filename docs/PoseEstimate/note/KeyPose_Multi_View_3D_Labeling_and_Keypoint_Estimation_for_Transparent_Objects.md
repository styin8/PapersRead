...
总的损失被定义为加权和 Ltotal = Lkp + αLproj + 0.001Lloc (5) Lloc 上的小权重将概率分布推向正确的局部形式，同时允许在必要时有散开的空间。为了稳定起见，对 Lproj 适用一个课程是很重要的。权重 α 在训练步骤的[1/3, 2/3]区间内从 0 升至 2.5，以使预测的 UVD 值稳定下来。否则，收敛会很困难，因为重投误差梯度最初可能非常大。
